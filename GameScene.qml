...
// Dungeons of Omakon — the game scene (host-agnostic).
// Two hosts load this file by relative URL:
//   * the Omarchy shell plugin — Panel.qml at the repo root (PanelWindow
//     overlay; runs persisted in the keyring via a secret-tool pipeline)
//   * the standalone desktop app — standalone/main.qml (ApplicationWindow;
//     runs persisted through the FileStorage C++ context property)
// The host wires itself in as the `hostIo` object before boot and provides:
//   hostIo.bootRun() / loadArchive() / storeRun(text) / storeArchive(text) /
//   clearRun() / closeWindow()
// The scene owns no window chrome, no persistence transport, and no
...
Item {
  ...
  // Theme compat: the plugin build pulled these from Omarchy's shell style.
  // Standalone ships the same palette inline so scene QML is unchanged.
  // Theme-palette compat shims. The plugin build reads these from the
  // Omarchy shell globals; standalone hardcodes the same colors.
  QtObject {
    id: colorShim
    readonly property var menu: ({
      background: "#15131a",
      border: "#3a3440",
      selectedBackground: "#241f2b",
      text: "#cfcace"
    })
  }
  QtObject {
    id: styleShim
    readonly property var font: ({ menuFamily: "JetBrainsMono Nerd Font" })
  }

  // Persistence backend, wired in by the host right after load (see the
  // hostIo contract in the file header). null until then; every IO call
  // site guards so a premature bootLoad() is a harmless no-op.
  property var hostIo: null
  ...
  // open()/close() live on the host (Panel.qml / main.qml), not here: the
  // scene has no window. In-scene close paths (Escape, title-bar X) call
  // hostIo.closeWindow() — the shell hides the panel window, the app quits.
  ...
  // ---- Save I/O (standalone build) ------------------------------------------
  // FileStorage (C++) keeps run/archive in JSON files under the platform
  // app-data dir (%APPDATA%/omakon on Windows, ~/.local/share/omakon on
  // Linux, ~/Library/Application Support/omakon on macOS), written with
  // QSaveFile atomic rename — replacing the shell build's secret-tool
  // keyring pipeline. The ioQueue indirection is retained so die()'s
  // store-archive-then-clear-run ordering is unchanged.

  // Static data tables — bundled via Qt resources so the app needs no
  // external data files on disk.
  ...
  // ---- Persistence (host-injected, see hostIo above) ----------------------
  // die() calls saveArchiveEntry() then clearRun() back-to-back; the
  // ordering survives because each host serializes its own jobs — the
  // shell's Process pipeline runs one job at a time, and the app's writes
  // are synchronous.
  function bootLoad() { if (!hostIo) return; hostIo.bootRun() }

  function loadArchive() { if (!hostIo) return; hostIo.loadArchive() }

  function saveRun() {
    if (mode !== "game") return
    if (!hostIo) return
    hostIo.storeRun(Save.serializeRun(currentState()))
  }

  function saveArchiveEntry(entry) {
    archive = Save.appendArchive(archive, entry)
    if (!hostIo) return
    hostIo.storeArchive(JSON.stringify(archive))
  }

  function clearRun() { if (!hostIo) return; hostIo.clearRun() }
  ...
  function confirmNewGame(name) {
    ...
    heroHp = 32; heroHpMax = 32; heroMp = 17; heroMpMax = 17
  ...
  // The host (Panel.qml / main.qml) owns the boot sequence and calls
  // bootLoad() once it has wired hostIo — see the host's scene Loader.
  ...
  // ---- Frame ---------------------------------------------------------------
  // Standalone build: main.cpp supplies the OS window (ApplicationWindow);
  // this Item's only job is the game frame + input focus. The PanelWindow /
  // WlrLayershell overlay shell of the plugin build is dropped entirely.
  Rectangle {
      id: frame
      anchors.fill: parent
      color: colorShim.menu.background
      border.color: colorShim.menu.border
      border.width: 2
      focus: true
      ...
      Keys.onEscapePressed: {
        ...
        else if (root.mode === "menu") hostIo.closeWindow()
        else hostIo.closeWindow()      // close always saves in game mode
      }

      // ']' — dump the vista trace (short lines, journalctl-friendly).
      // 'f' — (dev) force a Depth-1 encounter so the combat loop is testable.
      //
      // Arrow keys + WASD drive movement, mirroring the on-screen directional
      // pad: Up/W = step forward, Down/S = step back, Left/A = turn left,
      // Right/D = turn right.
      Keys.onPressed: function(event) {
        if (root.mode !== "game") return
        if (event.key === 0x5d /* ] */) { root.debugVista(); event.accepted = true; return }
        if (event.key === Qt.Key_F)     { root.debugSpawnEncounter(); event.accepted = true; return }

        // Movement: only when no modal/combat overlay has stolen focus.
        if (root.combat) return
        if (event.key === Qt.Key_Up    || event.key === Qt.Key_W) { root.move(0); event.accepted = true; return }
        if (event.key === Qt.Key_Down  || event.key === Qt.Key_S) { root.move(2); event.accepted = true; return }
        if (event.key === Qt.Key_Left  || event.key === Qt.Key_A) { root.turn(-1); event.accepted = true; return }
        if (event.key === Qt.Key_Right || event.key === Qt.Key_D) { root.turn(1);  event.accepted = true; return }
      }
      ...
      Rectangle {
        ...
        Text {
          ...
          color: colorShim.menu.text
          font.family: styleShim.font.menuFamily
          font.bold: true
          font.pixelSize: 13
        }
      ...
      Rectangle {
        ...
        Text {
          ...
          color: Qt.darker(colorShim.menu.text, 1.6)
          font.family: styleShim.font.menuFamily
          font.pixelSize: 10
        }
        ...
        component Arrow: Text {
          ...
          font.family: styleShim.font.menuFamily
          font.pixelSize: 22
          color: blocked ? Qt.darker(colorShim.menu.text, 2.2) : colorShim.menu.text
          MouseArea {
            anchors.fill: parent
            enabled: !blocked
            onClicked: {
              ...
              Item {
                ...
                Rectangle {
                  ...
                  color: !seen ? "black"
                    ...
                    : feat === "down" ? "#b09030"
                    : "#5b5548"
                  border.color: Qt.darker(colorShim.menu.border, 1.5)
                  border.width: 1
                }
        ...
        Rectangle {
          ...
          Rectangle {
            ...
            readonly property string tierColor: {
              if (!root.combatView) return colorShim.menu.text
              var c = root.combatView.color
              return c === "red" ? "#e06060"
                   : c === "yellow" ? "#e0c050" : colorShim.menu.text
            }
            ...
            Column {
              ...
              Rectangle {
                ...
                Text {
                  ...
                  font.pixelSize: 9; font.family: styleShim.font.menuFamily
                  color: colorShim.menu.text
                }
              }
            }
            Text {
              ...
              color: Qt.darker(colorShim.menu.text, 1.8)
              font.family: styleShim.font.menuFamily
              font.pixelSize: 9
            }
            MouseArea {
          ...
          Rectangle {
            ...
            Text {
              ...
              text: "COMBAT LOG"
              color: Qt.darker(colorShim.menu.text, 1.4)
              font.family: styleShim.font.menuFamily
              font.bold: true
              font.pixelSize: 10
            }
      ...
      Rectangle {
        ...
        Column {
          ...
          Repeater {
            ...
            Rectangle {
              ...
              width: 160; height: 34
              color: colorShim.menu.selectedBackground
              border.color: colorShim.menu.border
              border.width: 2
              anchors.horizontalCenter: parent.horizontalCenter
              Text {
        ...
        Column {
          ...
          Rectangle { height: 1; width: parent.width; color: colorShim.menu.border }
          Repeater {
            model: root.archive
            Row {
              property var e: modelData
              spacing: 16
              Text { text: e.name; color: colorShim.menu.text
                     font.family: styleShim.font.menuFamily; font.pixelSize: 12; width: 120 }
              Text { text: e.started || ""; color: Qt.darker(colorShim.menu.text, 1.4)
                     font.family: styleShim.font.menuFamily; font.pixelSize: 11; width: 90 }
              Text { text: "FLOOR " + e.floor; color: "#b09030"
                     font.family: styleShim.font.menuFamily; font.pixelSize: 12; width: 70 }
              Text { text: "LVL " + e.level; color: colorShim.menu.text
                     font.family: styleShim.font.menuFamily; font.pixelSize: 12; width: 60 }
              Text { text: "SCORE " + e.score; color: colorShim.menu.text
                     font.family: styleShim.font.menuFamily; font.pixelSize: 12 }
            }
          }
          Text {
            ...
            color: Qt.darker(colorShim.menu.text, 1.8)
            font.family: styleShim.font.menuFamily; font.pixelSize: 11
          }
          Rectangle {
            width: 120; height: 28
            color: colorShim.menu.selectedBackground
            ...
            Text { anchors.centerIn: parent; text: "BACK"
                   color: colorShim.menu.text; font.family: styleShim.font.menuFamily
        ...
        Text {
          ...
          color: Qt.darker(colorShim.menu.text, 1.6)
          font.family: styleShim.font.menuFamily; font.pixelSize: 13
        }
      }
        ...
        Row {
          ...
          Column {
            ...
            Rectangle {
              width: 44
              height: 40
              color: root.automapOn ? colorShim.menu.selectedBackground : "transparent"
              border.color: colorShim.menu.border
              border.width: 2
              Text {
                anchors.centerIn: parent
                text: "▦"
                color: root.automapOn ? colorShim.menu.text : Qt.darker(colorShim.menu.text, 2.0)
                font.family: styleShim.font.menuFamily
                font.pixelSize: 18
              }
              MouseArea {
          ...
          Column {
            ...
            Rectangle {
              width: 44
              height: 40
              color: root.popupMode === "stats" ? colorShim.menu.selectedBackground : "transparent"
              border.color: colorShim.menu.border
              border.width: 2
              Text {
                anchors.centerIn: parent
                text: "♥"
                color: root.popupMode === "stats" ? colorShim.menu.text : Qt.darker(colorShim.menu.text, 2.0)
                font.family: styleShim.font.menuFamily
                font.pixelSize: 18
              }
          ...
          Column {
            ...
            Rectangle {
              ...
              Text {
                ...
                color: root.leftHand ? colorShim.menu.text : Qt.darker(colorShim.menu.text, 2.2)
                font.family: "JetBrainsMono Nerd Font"
                font.pixelSize: 18
              }
              MouseArea {
          ...
          Column {
            ...
            Rectangle {
              width: 44
              height: 40
              color: root.popupMode === "spells" ? colorShim.menu.selectedBackground : "transparent"
              border.color: root.spells.length > 0 ? colorShim.menu.border : Qt.darker(colorShim.menu.border, 1.8)
              border.width: 2
              Text {
                anchors.centerIn: parent
                text: "✦"
                color: root.spells.length > 0 ? colorShim.menu.text : Qt.darker(colorShim.menu.text, 2.0)
                font.family: styleShim.font.menuFamily
                font.pixelSize: 18
              }
              MouseArea {
          ...
          Column {
            ...
            Rectangle {
              width: 44
              height: 40
              color: root.popupMode === "inventory" ? colorShim.menu.selectedBackground : "transparent"
              border.color: colorShim.menu.border
              border.width: 2
              Text {
                anchors.centerIn: parent
          ...
          Column {
            ...
            Rectangle {
              ...
              color: "transparent"
              border.color: armed ? colorShim.menu.border : Qt.darker(colorShim.menu.border, 1.8)
              border.width: 2
              Text {
                anchors.centerIn: parent
                text: "󰜍"  // md-run U+F070D
                color: parent.armed ? colorShim.menu.text : Qt.darker(colorShim.menu.text, 2.0)
                font.family: "JetBrainsMono Nerd Font"
                font.pixelSize: 18
              }
              MouseArea {
        ...
        Rectangle {
          ...
          Column {
            ...
            Text { text: "CHARACTER SHEET"; font.bold: true; font.pixelSize: 12
              font.family: styleShim.font.menuFamily; color: colorShim.menu.text }
            Rectangle { width: parent.width; height: 1; color: colorShim.menu.border }
            ...
            Text { text: "HP " + root.heroHp + "/" + root.heroHpMax
              ...
              font.pixelSize: 11; font.family: styleShim.font.menuFamily; color: colorShim.menu.text }

            // EXP bar, e.g. EXP [|||||  ] 75/100
            Text { text: "EXP"; font.pixelSize: 10; font.family: styleShim.font.menuFamily;
              color: Qt.darker(colorShim.menu.text, 1.6) }
            Rectangle {
              width: parent.width; height: 12
              color: "transparent"; border.color: colorShim.menu.border; border.width: 1
              Rectangle {
                anchors.left: parent.left; anchors.top: parent.top; anchors.bottom: parent.bottom
              ...
              Column {
                Repeater {
                  ...
                  Text {
                    ...
                    font.pixelSize: 12; font.family: styleShim.font.menuFamily; color: colorShim.menu.text
                  }
                }
              }
                ...
                Repeater {
                  ...
                  Text {
                    ...
                    text: eq.inst
                      ...
                      : eq.label + "  —"
                    color: eq.inst ? colorShim.menu.text
                                   : Qt.darker(colorShim.menu.text, 1.8)
                    font.pixelSize: 11; font.family: styleShim.font.menuFamily
                  }
                }
              }
            }
          ...
          Grid {
            ...
            Repeater {
              ...
              Rectangle {
                ...
                border.color: equipped ? root.gold : colorShim.menu.border
                border.width: 1
                Text {
                  anchors.centerIn: parent
                  ...
                  text: item
                    ? (root.iconOf(item) || "·") : "·"
                  color: item ? colorShim.menu.text : Qt.darker(colorShim.menu.text, 2.4)
                  font.family: "JetBrainsMono Nerd Font"
                  font.pixelSize: 14
                }
    ...
    Rectangle {
      ...
      Column {
        ...
        Text {
          ...
          font.bold: true; font.pixelSize: 12
          font.family: styleShim.font.menuFamily; color: "#b09030"
        }
        Text { text: "Next stat point at the next even level."
          font.pixelSize: 10
          font.family: styleShim.font.menuFamily; color: Qt.darker(colorShim.menu.text, 1.5) }
      }
      ...
      Column {
        ...
        Repeater {
          ...
          Rectangle {
            ...
            color: colorShim.menu.selectedBackground
            border.color: colorShim.menu.border; border.width: 1
            Row {
        ...
        Column {
            ...
            Rectangle {
              ...
              Row {
                ...
                Text {
                  ...
                  font.pixelSize: 9; font.family: styleShim.font.menuFamily
                  color: Qt.darker(colorShim.menu.text, 1.7)
                  anchors.verticalCenter: parent.verticalCenter
                }
              }
              MouseArea {
          ...
          Rectangle {
            ...
            Text {
              ...
              font.pixelSize: 10; font.family: styleShim.font.menuFamily
              color: Qt.darker(colorShim.menu.text, 1.4)
            }
            MouseArea {
              anchors.fill: parent
    ...
    Rectangle {
      ...
      Column {
        ...
        Text {
          ...
          color: Qt.darker(colorShim.menu.text, 2.0)
          font.family: styleShim.font.menuFamily
          font.pixelSize: 9
        }
      }
      ...
      Flickable {
        ...
        Column {
          ...
          Text {
            ...
            font.family: styleShim.font.menuFamily; color: root.gold
          }
          Text {
    ...
    Rectangle {
      ...
      Column {
        ...
        Text {
          ...
          color: Qt.darker(colorShim.menu.text, 1.3)
          font.family: styleShim.font.menuFamily
          font.pixelSize: 11
        }
