...
Item {
  ...
  // `storage` comes in as a context property set by main.cpp (visible here
  // because Panel.qml is loaded under the same QQmlEngine root context).
  ...
  function open() { if (mode === "loading") bootLoad() }
  // Standalone: window lives as long as the app; close() saves then quits.
  // try/catch kept (2026-08-30 lesson): a throwing close must never block
  // teardown.
  function close() {
    if (mode === "game") {
  ...
  // Synchronous pump: FileStorage is blocking and atomic (QSaveFile), so a
  // while-loop replaces the old Process.start/onExited round-trips. The
  // ioQueue indirection is kept only so die()'s archive-then-clear ordering
  // stays explicit.
  function ioFinished() {}

  function ioPump() {
    while (ioQueue.length > 0) {
      var job = ioQueue.shift()
      try {
        if (job.kind === "boot_run") {
          ...
          archive = Save.parseArchive(storage.readArchive())
        } else if (job.kind === "store") {
          var ok = (job.field === "run")
            ? storage.writeRun(job.text)
            : storage.writeArchive(job.text)
          if (!ok) console.log("omakon " + job.field + " save failed")
        } else if (job.kind === "run_clear") {
          storage.clearRun()
        }
      } catch (e) {
        console.log("omakon io job failed (" + job.kind + "): " + e)
      }
    }
  }
